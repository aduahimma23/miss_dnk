...
def home(request):
    ...
    header_image = Header.objects.filter().first()

    registration_status = RegistrationStatus.objects.first()
    if registration_status:
        registration_open = registration_status.is_registration_open
    else:
        registration_open = False  # Set a default value if RegistrationStatus is not found

    context = {
        'contestants': contestants,
        'next_year': next_year,
    }
    return render(request, 'main/index.html', {'header_image': header_image, 'registration_open': registration_open, 'context': context})
